...
class aplot:
    def __init__ (self, wl,spec, name, type = "veg", h= 8, w = 20, ReportType=[],LineColor=[],LineName=[],LineNM=[],LineAll=[],LineFound=[],ShadeName=[],ShadeColor=[],ShadeStart=[],ShadeStop=[],PltSave=0, folder_name=None):
    # Parameters:
    ...
    # - PltSave: Control how the plots are saved: 0 for none, 1 for printed on screen, 2 for saved as a png, 3 for saved and printed.
        self.spec = spec
        self.wl = wl
        self.type = type
        self.h = h
        ...
        max_length = max(len(LineName), len(LineNM), len(LineAll), len(LineFound),len(LineColor),len(ReportType))
        for n in range(max_length):
            ...
            if n < len(ReportType) and ReportType:  # Check if ReportType has an element at index n and is not empty
                args['reportType'] = ReportType[n]
...
>>>>>>> b1daa9fac1df0e2e262d9be19c0d727d08d7b9e1
        plt.xlim(min_wavelength, max_wavelength)

    def add_lines(self,color="green",nameBool=1,nmBool=0,allBool=1,foundBool=1,LineSize=0.05,fontSize=10,reportType=0):
    ...
    # - fontSize: Font size of annotations (default is 10).
        ...
        if reportType == 1:
            self.outputR.append(output_list)
        if reportType == 1:
    ...
    def save_plot(self):
        ...
        filename = f"{self.name}_{current_time}.png"
        ...
        if self.PltSave == 2:
            ...
            print(f"Plot saved as {filename} in folder {self.folder_name}")
        if self.PltSave == 3:
            plt.savefig(filepath)
            plt.show()
            print(f"Plot saved as {filename} in folder {self.folder_name}")
